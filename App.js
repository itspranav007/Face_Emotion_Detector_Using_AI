...
export App=()=> {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>AI Emotion Detector</Text>
        ...
        <View style={styles.resultContainer}>
          {emotion.error ? (
            ...
            Object.entries(emotion).map(([key, value]) => (
              <Text key={key} style={styles.emotionText}>{key.toUpperCase()}: {value.toFixed(2)}</Text>
            ))
          )}
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  button: { backgroundColor: '#6200EE', padding: 12, borderRadius: 8, marginBottom: 10 },
  buttonText: { color: '#FFF', fontSize: 16, fontWeight: 'bold' },
  image: { width: 250, height: 250, borderRadius: 10, marginVertical: 10 },
  resultContainer: { marginTop: 20, alignItems: 'center' },
  emotionText: { fontSize: 18, fontWeight: 'bold', marginTop: 5 },
